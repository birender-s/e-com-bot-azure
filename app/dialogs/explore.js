...
const extractQuery = (session, args) => {
    if (args && args.entities && args.entities.length) {
        ...
        return `${(detail || { entity: '' }).entity} ${(question || { entity: '' }).entity}`.trim();
...
const listCategories = (session, subcategories, start = 0) => {
    ...
    if (!more) {
        ...
        session.endDialog(`We ${start > 0 ? 'also ' : ''}have ${message} and ${start > 0 ? 'even ' : ''}more.` +
            (start > 0
...
const listProducts = (session, products, start = 0) => {
    ...
    if (start === 0) {
        session.send(`I found ${products.length} products and here are the best matches. Tap on the image to take a closer look.`);
    }
...
module.exports = function (bot) {
    bot.dialog('/explore', [
        function (session, args, next) {
            ...
            if (!query) {
                ...
                builder.Prompts.text(session, 'I am sorry, what would you like me to look up for you?');
            } else {
                next({ response: query });
            }
        },
    ...
    bot.dialog('/next', [
        function (session, args, next) {
            if (!session.privateConversationData || !session.privateConversationData.list) {
            ...
            session.endDialog('Something funny happened and I started wondering who I am');
        }
    ]);
}
